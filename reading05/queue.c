...
void queue_push(Queue *q, char *value) {
    ...
    // huh???
    /**
     * Okay, so after lecture i have learned that this is more deep C lore.
     * Instead of using a linked list to implement the bounded buffer, we can use
     * a regular int array and just keep track of the "writer" index and the "reader"
     * index. Basically, every time we write, we advance writer. We loop around by
     * modding the writer by the length of the bounded buffer. We do the same for the
     * reader to keep track of what we've consumed so far.
     */
